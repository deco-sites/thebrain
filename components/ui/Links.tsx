...
function Links(props: Props) {
  ...
  const logoOrIcon = header?.logo
    ...
    : <Icon id="Deco" size={150} />;

    const maybeLink = header?.logo?.link
    ? <a href={header?.logo?.link!} target="_blank">{logoOrIcon}</a>
    : logoOrIcon;

  const ColorsNeutralAndHover = {
    color: links.style.textColor,
    "--hover-background-image": `linear-gradient(to right, ${
      links.style.gradientColors.hover.map((color) => color.color).join(", ")
    })`,
